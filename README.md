...
When assessing the loss and accuracy learning curves of the model, it is evident that the model quickly reaches high performance on both the training and validation dataset (see figure 1). This is manifested by the training and validation accuracy curves reaching nearly 100% accuracy immediately after the first epoch, suggesting that the model quickly learns the training data and is not able to learn more after only a few epochs. Hence, the model performs very well on the specified task, and generalizes well on the validation data. Similarly, the training and validation loss start off low, suggesting that the model very quickly and effectively learns the task. While the training loss decreases in the beginning suggesting that the model is learning, it quickly reaches a loss of 0 suggesting that there is nothing left for the model to learn about the data. The relation between the validation loss and the training loss seems to indicate that the model might overfit the data and generalize less well on the validation data. This is indicated by the fact that the validation loss starts to increase around 12.5 epochs, suggesting that the model might be trained for too long. 
To sum up, the model seems to quickly learn the task and reach a high accuracy for both the training data and validation data. Hence, it might not be necessary to train the model for 20 epochs. 
...
Figure 1: Loss/accuracy learning curves of neural network model. <br>
